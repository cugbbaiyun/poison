...
class MINDIterator(BaseIterator):
    ...
    def update_poisoned_news(self, nid, words):
        poisoned_index = self.nid2index[nid]
        self.news_title_poisoned[poisoned_index] = words

    def line_generate(self, index):
        line = ""
        nid = self.index2nid[index]
        line += nid
        line += self.col_spliter
        vert = self.vert_poisoned[index]
        line += vert
        line += self.col_spliter
        subvert = self.subvert_poisoned[index]
        line += subvert
        line += self.col_spliter
        title = ""
        for c in self.news_title_poisoned[index]:
            title += c
            title += " "
        title = title.strip(' ')
        line += title
        line += self.col_spliter
        ab = self.ab_poisoned[index]
        line += ab
        line += self.col_spliter
        url = self.url_poisoned[index]
        line += url
        line += self.col_spliter
        else1 = self.url_poisoned[index]
        line += else1
        line += self.col_spliter
        else2 = self.url_poisoned[index]
        line += else2 
        line += "\n"
        return line
    def write_poisoned_news(self, news_file):
        with open(news_file, 'w+') as f:
            for c in range(1, len(self.nid2index_poisoned) + 1):
                line = self.line_generate(c)
                f.write(line)

    def init_poisoned_news(self, news_file):
        self.nid2index_poisoned = {}
        self.index2nid_poisoned = {}
        self.news_title_poisoned = [""]
        self.vert_poisoned = [""]
        self.subvert_poisoned = [""]
        self.ab_poisoned = [""]
        self.url_poisoned = [""]
        self.else1_poisoned = [""]
        self.else2_poisoned = [""]
        ...
        with tf.io.gfile.GFile(news_file, "r") as rd:
            for line in rd:
                nid, vert, subvert, title, ab, url, else1, else2 = line.strip("\n").split(
                    self.col_spliter
                )
                if nid in self.nid2index_poisoned:
                    continue
                self.vert_poisoned.append(vert)
                self.subvert_poisoned.append(subvert)
                self.ab_poisoned.append(ab)
                self.url_poisoned.append(url)
                self.else1_poisoned.append(else1)
                self.else2_poisoned.append(else2)
                index = len(self.nid2index_poisoned) + 1
                self.nid2index_poisoned[nid] = index
                self.index2nid_poisoned[index] = nid
                title = word_tokenize(title)
                self.news_title_poisoned.append(title)
        ...
        with tf.io.gfile.GFile(news_file, "r") as rd:
            for line in rd:
                ...
                if nid in self.nid2index:
                    continue
                index = len(self.nid2index) + 1
                # self.nid2index[nid] = len(self.nid2index) + 1
                self.nid2index[nid] = index
                self.index2nid[index] = nid
                title = word_tokenize(title)
        ...
        for news_index in range(len(news_title)):
            ...
            for word_index in range(min(self.title_size, len(title))):
                if title[word_index] in self.word_dict:
                    word_ind = self.word_dict[title[word_index].lower()]
                    self.news_title_index[news_index, word_index] = word_ind
                    if word_ind not in self.word_freq.keys():
                        self.word_freq[word_ind] = 0
                    self.word_freq[word_ind] += 1
    ...
    def load_data_from_file(self, news_file, behavior_file):
        ...
        if not hasattr(self, "news_title_index"):
            ...
            self.init_poisoned_news(news_file)

        if not hasattr(self, "impr_indexes"):
            # print("fk", behavior_file)
    ...
    def _convert_data(
        ...
        his_list = np.asarray(histories_list, dtype=np.int32)
        # impr_list = np.asarray(impr_list, dtype=np.str)
