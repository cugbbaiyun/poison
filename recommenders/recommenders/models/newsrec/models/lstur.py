...
class LSTURModel(BaseModel):
    ...
    def _build_lstur(self):
        ...
        # F = scorer([user_indexes, his_input_title, pred_input_title_one]) 
        # grad = K.gradients(F, scorer.trainable_weights)
        # self.test_grads = keras.Model(
        #     [user_indexes, hisi_input_title, pred_input_title_one], grad
        # )
        return model, scorer