...
from recommenders.models.newsrec.models.hessians import hessian_vector_product
...
class BaseModel:
    ...
    def __init__(
        ...
        # self.hession = self.build_hession()
        self.model_params = self.model.weights
        self.scorer_params = self.scorer.weights
        # self.v_placeholder = [tf.placeholder(tf.float32, shape=a.get_shape()) for a in self.model_params]
        self.loss = self._get_loss()
        self.train_optimizer = self._get_opt()

        self.model.compile(loss=self.loss, optimizer=self.train_optimizer)
        # self.get_gradient_from_model()
    ...
    # def get_gradient_from_model(self):
    #     self.model_weights_list = self.model.trainable_weights
    #     self.model_gradients_list = K.gradients(self.model.total_loss, self.model_weights_list) # gradient tensors
    #     self.input_tensor = [self.model.inputs,
    #                          self.model._targets,
    #                          K.learning_phase()
    #     ]
    #     self.out_gradients = K.function(inputs = self.input_tensor, outputs = [self.model_gradients_list])
